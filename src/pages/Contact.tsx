...
export default function Contact() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (response.ok) {
        // Siz xohlagan inglizcha muvaffaqiyat xabari
        alert("Your request has been sent successfully! We will contact you within 24 hours to confirm your appointment.");
        (e.target as HTMLFormElement).reset();
      } else {
        alert("Something went wrong. Please try again later.");
      }
    } catch (error) {
      alert("Connectivity issue. Please check your internet connection.");
    } finally {
      setLoading(false);
    }
  };
